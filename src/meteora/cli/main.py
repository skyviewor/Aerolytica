"""Meteora CLI — Textual TUI and command dispatcher."""
...
if os.environ.get("METEORA_ENABLE_KITTY_KEY") != "1":
...
TerminalImage = None
if os.environ.get("METEORA_TEXTUAL_IMAGE_PROTOCOL") != "0":
    try:
...
from meteora.cli.image_widget import (
    extract_image_paths,
    image_metadata,
    resolve_image_path,
...
from meteora.core.config import (
    MeteoraConfig,
    clear_llm_api_key,
    save_llm_profile,
)
from meteora.core.debug_log import configure_debug_logging, debug_log
...
from meteora.data.plans import set_session_id
from meteora.data.pricing import TokenTracker, context_window_for, format_cost, format_token_count
from meteora.core.types import Message
from meteora.i18n import is_supported_language, language_label, language_options, t
...
class MeteoraApp(App):
    ...
    def __init__(self, config: MeteoraConfig, persist_config: bool = True):
        super().__init__()
        self.config = config
    ...
    async def _handle_local_llm_setup(self, text: str, setup: dict) -> None:
        ...
        if not setup.get("api_key"):
            ...
            self._chat_log.append(f"Meteora:\n{message}")
            return
    ...
    def _handle_set_command(self, text: str) -> None:
        ...
        from meteora.toolbox.builtin_tools import set_max_tool_rounds
    ...
    def _handle_vision_command(self, text: str) -> None:
        from meteora.data.vision_models import is_valid_vision_model, vision_model_options
    ...
    def _handle_mode_command(self, text: str) -> None:
        from meteora.data.modes import MODE_LABELS, MODE_OPTIONS
    ...
    def _set_mode(self, mode: str) -> None:
        ...
        self._refresh_mode_ui()
        from meteora.data.modes import MODE_LABELS
        self._set_footer_status(
            t("app.mode_switched", lang).format(mode=MODE_LABELS.get(mode, mode))
        )

    def action_cycle_mode(self) -> None:
        from meteora.data.modes import MODE_ORDER
    ...
    async def _generate_session_title(self, session_id: str) -> None:
        try:
            ...
            client = None
            try:
                from meteora.agent.llm_client import LLMConfig, LLMClient
    ...
    async def _handle_instructions_command(self, text: str) -> None:
        from meteora.data.instructions import load_instructions, clear_instructions, GLOBAL_INSTRUCTIONS_PATH
        ...
        if tool_name == "ensure_runtime_tools":
            ...
            if lang == "en":
                return (
                    ...
                    "Meteora needs these local commands to continue:\n"
                    f"  {tools}\n\n"
                    "What will change\n"
                    "  Create or update the conda environment: meteora-agent\n"
                    "  Install mamba inside meteora-agent if it is missing, then use it for speed\n"
            ...
            return (
                "← 安装命令行工具\n\n"
                "Meteora 需要先补齐这些本机命令，才能继续处理数据：\n"
                f"  {tools}\n\n"
                "将会做什么\n"
                "  创建或更新 conda 环境：meteora-agent\n"
                "  如果缺少 mamba，会把 mamba 安装到 meteora-agent 内并用它加速依赖解析\n"
    ...
    def _init_agent(self) -> None:
        try:
            from meteora.agent.loop import AgentLoop
            from meteora.toolbox.builtin_tools import download_era5  # noqa: F401
...
def _load_config() -> MeteoraConfig:
    cwd = Path.cwd()
...
def _save_config(config: MeteoraConfig) -> None:
    cwd = Path.cwd()
    for parent in [cwd, *cwd.parents]:
        config_path = parent / "meteora.yaml"
        if config_path.exists():
            ...
            return
    config.save(cwd / "meteora.yaml")


def _preferences_path() -> Path:
    override = os.environ.get("METEORA_PREFERENCES_PATH")
    if override:
        return Path(override)
    return Path.home() / ".meteora" / "preferences.yaml"
...
def _config_needs_llm_setup(config: MeteoraConfig) -> bool:
...
def _infer_llm_provider_model(text: str, config: MeteoraConfig) -> tuple[str, str, str]:
    lowered = text.lower()
...
def _parse_llm_setup_from_text(text: str, config: MeteoraConfig) -> dict | None:
...
def _apply_llm_setup(config: MeteoraConfig, setup: dict) -> None:
...
def _clear_llm_setup(config: MeteoraConfig, *, reset_provider: bool = False) -> None:
...
def _llm_setup_success_message(config: MeteoraConfig) -> str:
...
def _llm_clear_success_message(
    config: MeteoraConfig, *, reset_provider: bool = False
) -> str:
    provider_name = _display_provider_name(config.llm.provider)
    if reset_provider:
        return (
...
def _llm_auth_setup_message(config: MeteoraConfig) -> str:
    provider_name = _display_provider_name(config.llm.provider)
...
def _display_vision_model_name(model: str) -> str:
    from meteora.data.vision_models import VISION_MODELS
...
def _vision_configured(config: MeteoraConfig) -> bool:
...
def _subagent_task_prompt(task: SubAgentTask) -> str:
    return (
        "你是 Meteora 的后台子 agent。只专注完成下面这个后台任务，"
...
def _session_title_prompt(messages: list[Message], language: str) -> str:
    ...
    for msg in messages:
        ...
        if msg.role == "user" and seen_user:
            break
        role = "用户" if msg.role == "user" else "Meteora"
...
def _run_llm_setup_wizard(config: MeteoraConfig) -> bool:
    provider = "deepseek"
    preset = get_provider_preset(provider)
    ...
    print()
    print("后续如果要更换其他模型，可以直接用自然语言描述，让 Meteora 替你完成配置。")
    ...
    print("API key 已保存到 ~/.meteora/secrets.yaml")
    print()
    return True
...
def main():
    ...
    standard_log_path = log_path.with_name("meteora.log")
...
def _print_usage():
    print("""
Meteora — 气象科研 AI Agent IDE

命令:
  meteora init            初始化当前目录的配置与工作目录
  meteora chat            启动 Textual TUI 对话（支持中文输入和流式输出）
  meteora chat --simple   启动纯文本对话
  meteora chat --mouse    启用 Textual 鼠标滚轮（默认已启用，保留兼容）
  meteora chat --no-mouse 禁用 Textual 鼠标模式，交给终端原生选择/复制
  ↑/↓ 或 PageUp/PageDown  对话中滚动聊天区域
  /copy                   对话中复制最后一条 Meteora 回复到剪贴板
  ...
  meteora help            显示此帮助信息
""")


async def _chat_simple(config: MeteoraConfig):
    from meteora.agent.loop import AgentLoop
    from meteora.toolbox.builtin_tools import download_era5  # noqa: F401
    ...
    try:
        while True:
            ...
            if user_input == "/provider" or user_input.startswith("/provider "):
                ...
                preset = get_provider_preset(provider)
                if preset is None:
                    print("未知模型服务商。请使用内置服务商，或在 meteora.yaml 中自定义。")
                    continue
                previous_provider = config.llm.provider
                config.llm.apply_active_provider_defaults()
                config.llm.switch_provider(provider)
...
def _init():
    from meteora.cli.init_runtime import setup_runtime

    cwd = Path.cwd()
    config_path = cwd / "meteora.yaml"
    ...
    print(f"在当前目录初始化 Meteora 工作区: {cwd}")
