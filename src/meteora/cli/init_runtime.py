"""Interactive runtime setup used by ``meteora init``."""
...
def setup_runtime() -> bool:
    ...
    if conda is None:
        if not _confirm("未找到 conda。是否安装 Miniconda？[Y/n] ", default=True):
            print("已跳过 Miniconda 和 meteora-agent 环境安装。")
            return False
    ...
    if not ensure_meteora_agent(conda):
        return False
...
def ensure_meteora_agent(conda: Path) -> bool:
