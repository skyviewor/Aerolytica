...
export DYLD_LIBRARY_PATH=~/miniconda3/envs/meteora-agent/lib:$DYLD_LIBRARY_PATH
```
...
Some tools are only available via pip. They can also be installed into the meteora-agent sandbox:

```bash
conda run -n meteora-agent pip install <package>
```
...
- `defaults` channel: `conda install -n meteora-agent <package> -y`
- `pip`: `conda run -n meteora-agent pip install <package>`
