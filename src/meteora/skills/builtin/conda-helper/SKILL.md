---
name: conda-helper
description: Use when executing shell commands that fail because a CLI tool is missing, or when a download/data-processing tool returns a "tool_missing" error. Covers installing missing tools into the unified meteora-agent conda sandbox on demand — never pre-install.
---
...
**Install on demand, never pre-install.** Meteora starts with nothing. Only when a shell command fails (`command not found`) or a tool returns `tool_missing`, detect, propose, install, and retry.

## meteora-agent Unified Sandbox

All Meteora runtime CLI tools (NCO, CDO, eccodes, GDAL, netcdf toolchain, etc.) go into **one** `meteora-agent` conda environment. Not one env per tool.
...
### Step 2: Check or create meteora-agent
...
```
conda create -n meteora-agent -c conda-forge python -y
```

This is the only kind of base-environment operation allowed: creating or
updating the `meteora-agent` environment by name. Never install runtime packages
into base.

### Step 3: Install mamba inside meteora-agent

Use `mamba` for runtime package installs because it resolves conda-forge
dependencies faster than conda. But mamba itself must live in `meteora-agent`,
not in base.

If `~/miniconda3/envs/meteora-agent/bin/mamba` is missing:

```
conda install -n meteora-agent -c conda-forge mamba -y
```

If this fails, fall back to plain `conda install -n meteora-agent ...`; do not
...
**First time (meteora-agent does not exist):**
```
conda create -n meteora-agent -c conda-forge python -y
```

**Append (meteora-agent already exists):**
```
~/miniconda3/envs/meteora-agent/bin/mamba install -p ~/miniconda3/envs/meteora-agent -c conda-forge <package> -y
```

**Multiple packages at once (avoids redundant dependency resolution):**
```
~/miniconda3/envs/meteora-agent/bin/mamba install -p ~/miniconda3/envs/meteora-agent -c conda-forge nco cdo -y
```

**Fallback if mamba is unavailable inside meteora-agent:**
```
...
4. **One sandbox** — all tools go into `meteora-agent`; do not create per-tool environments
5. **Prefer env-local mamba** — install and use mamba only inside `meteora-agent`
...
- pip-only packages — install via `conda run -n meteora-agent pip install <pkg>`
