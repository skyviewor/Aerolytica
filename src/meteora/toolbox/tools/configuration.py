...
from meteora.toolbox.config_access import find_config, find_config_path, mask_secret
...
@register_tool(
    ...
    description=(
        "列出 Meteora 内置支持的 LLM 提供商、默认模型和 API key 获取入口。"
...
def clear_cds_config() -> dict:
    ...
    from meteora.core.config import CDSCredentials

    config_path = None
    cwd = Path.cwd()
    for parent in [cwd, *cwd.parents]:
        p = parent / "meteora.yaml"
        if p.exists():
            config_path = p
            break

    config = find_config() if config_path is not None else MeteoraConfig.create_default()
