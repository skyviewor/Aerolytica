...
def find_project_dir() -> Path:
    """Walk up from cwd to find the Meteora project root."""
    cwd = Path.cwd()
    for parent in [cwd, *cwd.parents]:
        if (parent / "meteora.yaml").exists():
            return parent
        if (parent / "pyproject.toml").exists() and (parent / "src" / "meteora").exists():
            return parent
        if (parent / ".git").exists() and (parent / "src" / "meteora").exists():
            return parent
    return cwd
