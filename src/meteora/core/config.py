"""Meteora workspace configuration."""
...
class MeteoraConfig(BaseModel):
    ...
    @classmethod
    def load(cls, path: Path | str) -> "MeteoraConfig":
        path = Path(path)
        if not path.exists():
    ...
    @classmethod
    def create_default(cls) -> "MeteoraConfig":
...
def user_secrets_path() -> Path:
    override = os.environ.get("METEORA_SECRETS_PATH")
    if override:
        return Path(override)
    return Path.home() / ".meteora" / "secrets.yaml"
...
def load_user_secrets() -> dict:
    ...
    try:
        ...
        return {}
    data = MeteoraConfig._resolve_env(data)
...
def apply_user_secrets(config: MeteoraConfig) -> MeteoraConfig:
